...
const ICONS: Record<string, string> = {
  ...
  food:
    '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 9V2H9v7H7V2H5v7H3v2h2v7a2 2 0 002 2v1a2 2 0 002 2v-1a2 2 0 002-2v-7h2V9h-2zm8 0V2h-2v7h-2V2h-2v7h-2v2h2v7a2 2 0 002 2v1a2 2 0 002 2v-1a2 2 0 002-2v-7h2V9h-2z" /></svg>',
  pgrs:
    '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3v18h18M7 14l3-3 3 2 4-5" /></svg>',
  exercise:
    '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>',
  insights:
    '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 3a3.75 3.75 0 00-3.75 3.75v.5A3.25 3.25 0 002.75 10.5v4A3.25 3.25 0 006 17.75h.5A3.75 3.75 0 0010.25 21h3.5a3.75 3.75 0 003.75-3.25h.5a3.25 3.25 0 003.25-3.25v-4A3.25 3.25 0 0018 7.25h-.5A3.75 3.75 0 0013.75 3h-4z" /></svg>',
  cloud:
    '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 010-8 5 5 0 019.584-1.757A4.5 4.5 0 1118 16H7zm5-4v6m0 0l-3-3m3 3l3-3" /></svg>',
};
...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    description: {
      zh: '通过餐前餐后血糖配对分析（血糖指数），识别友好食物与风险食物。',
      en: 'Use glucose index pairing analysis to identify glucose-friendly foods and foods that need caution.',
      ja: '血糖指数ペアリング分析を使用して、血糖に優しい食品と注意が必要な食品を特定します。',
      'zh-Hant': '透過餐前餐後血糖配對分析（血糖指數），識別友好食物與風險食物。',
    },
    icon: ICONS.pgrs,
  },
  {
...
export function getFeatureIcon(id: string): string {
  return ICONS[id] || ICONS.glucose;
}
